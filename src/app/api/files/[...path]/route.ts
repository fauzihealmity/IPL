...
export async function GET(
  ...
  try {
    ...
    if (relativePath.startsWith("payment-proofs/")) {
      ...
      if (
        !isAdmin &&
        proof.payment.residentId !== session.user.residentId
      ) {
      ...
      if (
        !isAdmin &&
        receipt.payment.residentId !== session.user.residentId
      ) {
        throw new ForbiddenError(
          "Anda tidak memiliki akses ke berkas ini."
        );
      ...
      if (
        !isAdmin &&
        update.complaint.residentId !== session.user.residentId
      ) {
        throw new ForbiddenError(
          "Anda tidak memiliki akses ke berkas ini."
        );
    ...
    } else if (relativePath.startsWith("advertisements/")) {
      // Advertisement banners are broadly visible to authenticated users.
      // No per-record ownership check is needed because advertisement
      // banners are intended to be displayed to authenticated residents
      // and administrators.
    } else {
      return NextResponse.json(
        { success: false, message: "Berkas tidak ditemukan." },
